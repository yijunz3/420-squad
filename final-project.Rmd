...
```
```{r, echo = FALSE}
plot(Price ~ Kilometers_Driven, data = original, pch = 20, col = "darkgreen",main = "Price vs Kilometes_Driven", cex = 1)
...
ncol(ucar)
```
Then the data contains 10 variables and 5759 entries.
...
The modelling process can start with a full addiditive model with backward aic for a good model determination.
```{r}
...
The next step is to develop a full first order and second order polynomial model with backward aic for a good model determination.
```{r}
...
Finally, an full interaction model with backward aic for a good model determination can be developed.
```{r}
