...
We’ll try adding some polynomial transformations to the predictors, but also keep the log transform of the response:
...
### Model Selection
To begin with, we calculate the LOOCV RMSE for each model:
```{r}
...
```
Therefore, we would like to select the model `log_car_aic` to be the final model, since it achieves the smallest LOOCV RMSE with 15 predictors, which is not a very large number.
