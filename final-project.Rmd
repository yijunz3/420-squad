...
## Discussion
For the 'log_add_aic', we observe that 'Mileage' would be the most important predictor.
```{r}
log_car = lm(log(Price) ~ ., data = ucar)
log_car_aic = step(log_car, direction = "backward", trace = 0)
...
summary(log_car_aic)$coefficients[summary(log_car_aic)$coefficients[ ,4] < 0.05,]
summary(log_car_aic)$r.squared
```

```{r}
anova(log_car_aic, log_car, test = "F")
```

The p-value of the F test is smaller than $a = 0.05$. Therefore, we should reject the null and conclude that the model is significant in full for explaining used cars' prices.
